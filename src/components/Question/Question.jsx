            ...
            <div>
                <form onSubmit={handleSubmit}>
                    <span className='all-options'>
                        {options.map((option, id) => <h4 key={id} onClick={(event) => setSelectedAnswer(seperateOptionText(event.target.innerText, String.fromCharCode(id + 65)))} className={`${selectedAnswer === option ? 'single-option selected-option' : "single-option"}`}>
                    ...
                    <div className='check-see-answer'>
                        ...
                        <CheckAnswer answer={correctAnswer} selectedAnswer={selectedAnswer}></CheckAnswer>
                    </div>
                    <div className='prev-next'>
                        ...
                        <span className={`${questionNo === totalQuestion - 1 ? '' : "d-none"}`}>
                            <button type="submit"
                                className='submit'
                                onClick={() => { setSubmit(true); handleSelectedAnswers(questionId, selectedAnswer) }}
                            //disabled={selectedAnswer === ""}
                            >Submit</button>
                        </span>
                    </div>
